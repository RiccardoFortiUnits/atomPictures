...
for distance in atomDistance:
	...
	#____________________________NEW_________________________________________
	freeFlightTime = 1e-6#the imaging laser beams won't be active until this time (this time is counted inside experimentDuration)

	'''-------------------------------folders and files------------------------------'''
	pictureFolder = f"D:/simulationImages/feeFall{freeFlightTime*1e6}us_Yt{isotope}_{int(experimentDuration*1e6)}us_{nOfAtoms}tweezerArray{usedCase}/{int(acquisitionDuration*1e6)}us_pictures{pixelType}/"
	simulationFolder = f"D:/simulationImages/feeFall{freeFlightTime*1e6}us_Yt{isotope}_{int(experimentDuration*1e6)}us_{nOfAtoms}tweezerArray{usedCase}/simulation/"
	...
	for repeat in range(1000):
		...
		if not os.path.exists(simulationFileName):
			...
			#____________________________NEW_________________________________________
			b_firstHalfPeriod = trajlib.Laser(0,	0, Lambda, imgPower, (imgWaist/2,imgWaist/2), switchingTimes =      [freeFlightTime							, impulseDuration/2, experimentDuration])
			b1 = 				trajlib.Laser(np.pi,0, Lambda, imgPower, (imgWaist/2,imgWaist/2), switchingTimes = 		[freeFlightTime + impulseDuration / 2 	, impulseDuration, impulseDuration])
			b = 				trajlib.Laser(0,	0, Lambda, imgPower, (imgWaist/2,imgWaist/2), switchingTimes =      [freeFlightTime + impulseDuration *3 / 2, impulseDuration, impulseDuration])
			exp.add_laser(b_firstHalfPeriod)
			exp.add_laser(b)
			exp.add_laser(b1)
